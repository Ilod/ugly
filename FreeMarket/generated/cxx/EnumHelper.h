...
namespace FreeMarket
{
    ...
    public:
        ...
        explicit operator bool() { return static_cast<std::underlying_type<T>::type>(value) != 0; };
    ...
    {
        return EnumBoolAutoCast<T>(static_cast<T>(static_cast<std::underlying_type<T>::type>(val) & static_cast<std::underlying_type<T>::type>(oVal)));
    }
    ...
    {
        return EnumBoolAutoCast<T>(static_cast<T>(~static_cast<std::underlying_type<T>::type>(val)));
    }
    
    template<typename T> typename std::enable_if<IsFlagEnum<T>::value, bool>::type operator!(T val)
    {
        return static_cast<std::underlying_type<T>::type>(val) != 0;
    }
    
    template<typename T> typename std::enable_if<IsFlagEnum<T>::value, T&>::type operator&=(T& val, T oVal)
    {
        val = static_cast<T>(static_cast<std::underlying_type<T>::type>(val) & static_cast<std::underlying_type<T>::type>(oVal));
        return val;
    }
